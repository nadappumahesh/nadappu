<?php
	...
	class categort_posts extends WP_Widget {
		...
		function categort_posts() {
			...
			$this->WP_Widget( 'categort-posts-widget',Cherry .' - Category Posts', $widget_ops, $control_ops );
		}
		...
		function widget( $args, $instance ) {
			...
			<ul>
			  <?php cherry_last_posts_cat($numb_posts , $thumb , $cats_id )?>
			</ul>
			<div class="clear"></div>
		...
		function form( $instance ) {
			$defaults = array( 'cat_posts_title' =>__( 'Category Posts' , 'bd'), 'numb_posts' => '5' , 'cats_id' => '1' , 'thumb' => 'true' );
			...
			<p>
			  <label for="<?php echo $this->get_field_id( 'thumb' ); ?>"><?php _e('Display Thumbinals','bd') ?> : </label>

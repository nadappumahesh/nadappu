<?php
	...
	class video_widget extends WP_Widget {
		function video_widget()
		{
			...
			$this->WP_Widget( 'video-widget',Cherry .' - Video', $widget_ops, $control_ops );
		}
		...
		function widget( $args, $instance ) {
			...
			<?php elseif ( $youtube_video ):?>
				<div class="widget_video">
				  <iframe width="100%" height="210" src="http://www.youtube.com/embed/<?php echo $youtube_video ?>?rel=0" frameborder="0"></iframe>
				</div>
		...
		function form( $instance ) {
			$defaults = array( 'title' =>__(' Featured Video', 'bd') );
			...
			<p>
			  ...
			  <textarea id="<?php echo $this->get_field_id( 'embed_code' ); ?>" name="<?php echo $this->get_field_name( 'embed_code' ); ?>" class="widefat" ><?php if(isset($instance['embed_code'])){ echo $instance['embed_code']; } ?></textarea>
			</p>
			...
			<p>
			  ...
			  <input id="<?php echo $this->get_field_id( 'youtube_video' ); ?>" name="<?php echo $this->get_field_name( 'youtube_video' ); ?>" value="<?php if(isset($instance['youtube_video'])){ echo $instance['youtube_video']; } ?>" class="widefat" type="text" />
			  <small>if video url : http://www.youtube.com/watch?v=17vEq7rgzPE  Enter above <strong>17vEq7rgzPE</strong></small>
			</p>
			<p>
			  <label for="<?php echo $this->get_field_id( 'vimeo_video' ); ?>">Vimeo Video ID : </label>
			  <input id="<?php echo $this->get_field_id( 'vimeo_video' ); ?>" name="<?php echo $this->get_field_name( 'vimeo_video' ); ?>" value="<?php if(isset($instance['vimeo_video'])){ echo $instance['vimeo_video']; } ?>" class="widefat" type="text" />
			  <small>if video url : http://vimeo.com/16198681  Enter above <strong>16198681</strong></small>
			</p>
			<?php
		}
	}
